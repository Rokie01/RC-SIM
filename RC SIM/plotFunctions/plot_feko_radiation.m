function plot_feko_radiation(varargin)
  
  spec = '%*f %*f %*f %*f %*f %*f %*f %*f %f %*[^\n]';
  fid = fopen('input.out');
  tline = fgetl(fid);
  lineCounter = 1;
  i = 0;
  while ischar(tline)
      if strfind(tline, 'LOCATION          ETHETA             EPHI              directivity in dB             POLARISATION')
        i = i + 1;
        %tline = fgetl(fid);
        %tline = fgetl(fid);
        tline = fgetl(fid);
  ...
  radiation_plot.fig = figure('NumberTitle','off','Name','FEKO Radiation Plot', 'Position', [100 80 900 600]);
  radiation_plot.axes = axes('Units', 'Pixels',...
 'Title', 'FEKO Radiation Plot', 'position', [50 150 400 400],...
